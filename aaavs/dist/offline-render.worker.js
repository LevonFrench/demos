...
function registerAvsBasicTransforms(registry = new AvsEffectRegistry()) {
  ...
  registry.registerBuiltin(5, (context) => {
    if (context.preinit) return;
    const color = int3(context, 0, 16777215) & 16777215;
    const average = int3(context, 4, 0) !== 0;
    const every = int3(context, 8, 1);
  ...
  registry.registerBuiltin(11, (context) => {
    if (context.preinit) return;
    const enabled = int3(context, 0, 1);
    if (enabled === 0) return;
    const normal = [int3(context, 4, 8), int3(context, 8, -8), int3(context, 12, -8)];
  ...
  registry.registerBuiltin(12, (context) => {
    if (context.preinit) return;
    const mode = int3(context, 0, 1);
    if (mode === 0) return;
    const source = int3(context, 4, 2105376) & 16777215;
    const replacement = int3(context, 8, source) & 16777215;
    const distanceSquared = Math.pow(int3(context, 12, 10) * 2, 2);
  ...
  registry.registerBuiltin(23, (context) => {
    if (context.preinit || int3(context, 0, 1) === 0) return;
    const normalX = int3(context, 4, 1);
    const normalY = int3(context, 8, 1);
    const color = int3(context, 12, 0) & 16777215;
    const additive = int3(context, 16, 0) !== 0;
    const average = int3(context, 20, 0) !== 0;
    const onBeat = int3(context, 24, 0) !== 0;
    const beatX = int3(context, 28, normalX);
    const beatY = int3(context, 32, normalY);
    const duration = int3(context, 36, 4);
  ...
  registry.registerBuiltin(30, (context) => {
    if (context.preinit || int3(context, 0, 1) === 0) return;
    const quality = int3(context, 4, 50);
    const beatQuality = int3(context, 8, quality);
    const additive = int3(context, 12, 0) !== 0;
    const average = int3(context, 16, 0) !== 0;
    const onBeat = int3(context, 20, 0) !== 0;
    const duration = Math.max(1, int3(context, 24, 15));
...
function int3(context, offset, fallback) {
...
function renderDynamicMovement(context, config, state) {
  const source = config.buffer === 0 ? context.input : context.buffers.get(config.buffer - 1, context.input.width, context.input.height, false);
  if (!source) return;
  const vm = state.vm;
  vm.setHost({
...
function registerAvsFinalLowCountBuiltins(registry = new AvsEffectRegistry()) {
  ...
  registry.registerBuiltin(9, (context) => {
    if (context.preinit) return;
    const zoom = int4(context, 0, 31), direction = int4(context, 4, 31), blend2 = int4(context, 8, 0) !== 0;
    const beatReverse = int4(context, 12, 0) !== 0, beatSpeed = int4(context, 16, 0);
    const beatZoom = int4(context, 20, 31), beatScale = int4(context, 24, 0) !== 0, subpixel = int4(context, 28, 0) !== 0;
  ...
  registry.registerBuiltin(19, (context) => {
    ...
    if (!state) {
      state = createFountain(int4(context, 28, 0) / 32);
      fountainStates.set(context.component.path, state);
    }
    renderFountain(context, state, int4(context, 0, 16), readFiveColors(context), int4(context, 24, -20));
  });
  registry.registerBuiltin(41, (context) => {
    if (context.preinit || int4(context, 0, 1) === 0) return;
    const count = Math.max(0, Math.min(8, int4(context, 4, 2)));
    if (count === 0) return;
    let state = interferenceStates.get(context.component.path);
    if (!state) {
      state = { rotation: int4(context, 8, 0), status: Math.PI };
      interferenceStates.set(context.component.path, state);
    }
    const onBeat = int4(context, 48, 1) !== 0;
    if (onBeat && context.beat && state.status >= Math.PI) state.status = 0;
    const wave = Math.sin(state.status);
    const rotationIncrement = int4(context, 20, 0) + Math.trunc((int4(context, 40, 25) - int4(context, 20, 0)) * wave);
    const alpha = int4(context, 16, 128) + Math.trunc((int4(context, 36, 192) - int4(context, 16, 128)) * wave);
    const distance = int4(context, 12, 10) + Math.trunc((int4(context, 32, 32) - int4(context, 12, 10)) * wave);
    ...
    interference(context, points, alpha, int4(context, 44, 1) !== 0);
    ...
    const additive = int4(context, 24, 0) !== 0, average = int4(context, 28, 0) !== 0;
...
function renderFountain(context, state, velocity, colors, tilt) {
  const columns = 30;
...
function readFiveColors(context) {
  ...
  return defaults.map((fallback, index) => int4(context, 4 + index * 4, fallback) & 16777215);
}
...
function registerAvsLowCountBuiltins(registry = new AvsEffectRegistry()) {
  ...
  registry.registerBuiltin(0, (context) => {
    ...
    renderSimple(context, int5(context, 0, 40), color);
  });
  registry.registerBuiltin(14, (context) => {
    if (context.preinit) return;
    const count = int5(context, 4, 1);
    ...
    renderRing(context, int5(context, 0, 40), cycleColor(colors, state), int5(context, tail, 8), int5(context, tail + 4, 0));
  });
  registry.registerBuiltin(17, (context) => {
    ...
    const spacing = Math.max(2, int5(context, tail, 8));
    ...
    const color = cycleColor(colors, state);
    const blend2 = int5(context, tail + 12, 3);
    ...
    state.x += int5(context, tail + 4, 128);
    state.y += int5(context, tail + 8, 128);
  });
  registry.registerBuiltin(24, (context) => {
    ...
    const staticGrain = int5(context, 16, 0) !== 0;
    const threshold = Math.trunc(int5(context, 12, 100) * 255 / 100);
    for (let i = 0; i < context.input.pixels.length; i++) {
      const pixel = context.input.pixels[i];
      if (pixel === 0) continue;
      let gate;
      ...
      context.input.pixels[i] = int5(context, 4, 0) !== 0 ? blendPixel(grain, pixel, "additive") : int5(context, 8, 0) !== 0 ? blendPixel(grain, pixel, "average") : grain;
    }
  });
  return registry;
}
function renderSimple(context, effect, color) {
  const width = context.input.width;
  const height = context.input.height;
...
function renderRing(context, effect, color, size, spectrumMode) {
  const channel = effect >>> 2 & 3;
...
function colorList(context, offset) {
  return readColors(context, offset + 4, int5(context, offset, 1));
}
function readColors(context, offset, count) {
  if (count < 1 || count > 16) return [];
  const result = [];
  for (let i = 0; i < count && offset + i * 4 + 4 <= context.component.payload.length; i++) result.push(int5(context, offset + i * 4, 0) & 16777215);
  return result;
}
...
function multiplyColors(context, mode) {
...
function renderTexer(context, config, bitmap) {
  ...
  for (let y = 0; y < context.input.height; y++) {
    for (let x = 0; x < context.input.width; x++) {
      const mask = source[y * context.input.width + x] & 16777215;
      if (mask === 0) continue;
...
function rgbScanlineBytes(width, height) {
  assertRaster(width, height);
  return height * (1 + width * 3);
}
function packRgbScanlines(pixels, width, height, output) {
...
async function encodeRgb24Png(scanlines, width, height) {
...
async function sha256Hex(bytes) {
  const input = bytes.buffer instanceof ArrayBuffer ? bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength) : bytes.slice().buffer;
...
function assertRaster(width, height) {
...
async function run(message) {
  ...
  const createSeededRegistry = (seed) => {
    let randomState = seed || 1831565813;
    return createAvsCompatibilityRegistry({
      randomInt(maximum) {
        randomState = xorshift324(randomState);
  ...
  scope.postMessage({ type: "started", jobId: message.jobId, frameCount });
  const encodeAndWrite = async (slot, frame, sampleStart, sampleEnd) => {
    ...
    if (now - lastProgressAt >= 100 || writtenFrames === frameCount) {
      ...
      scope.postMessage({
        ...
        writtenFrames,
        frameCount,
        queueDepth: pendingCount(slots),
        elapsedMs: now - startedAt,
        renderMs: renderMsTotal,
        encodeWriteMs: encodeWriteMsTotal
      });
    }
  };
  for (let frame = 0; frame < frameCount; frame++) {
    ...
    if (cancelled) break;
    const slot = slots[frame % slots.length];
    if (slot.pending) await slot.pending;
    slot.pending = null;
    if (cancelled) break;
    while (cueIndex + 1 < message.presetCues.length && message.presetCues[cueIndex + 1].frame <= frame) {
      cueIndex++;
      ...
      runtime = new AvsCompatibilityRuntime(
        new Uint8Array(presetBytes),
        message.width,
        message.height,
        createSeededRegistry(cue.seed)
      );
    ...
    fillFramePcm(left, right, sampleStart, sampleEnd, pcmLeft, pcmRight);
    const renderStarted = performance.now();
    const rendered = runtime.renderPcm({ left: pcmLeft, right: pcmRight });
    const transitionProgress = transitionFrames > 0 ? Math.min(1, (frame - transitionStartFrame) / transitionFrames) : 1;
    if (outgoingRuntime && transitionProgress < 1) {
      const outgoing = outgoingRuntime.renderPcm({ left: pcmLeft, right: pcmRight });
      blendPackedRgb(outgoing.framebuffer.pixels, rendered.framebuffer.pixels, transitionProgress, transitionPixels);
      packRgbScanlines(transitionPixels, message.width, message.height, slot.scanlines);
    } else {
      outgoingRuntime = null;
      packRgbScanlines(rendered.framebuffer.pixels, message.width, message.height, slot.scanlines);
    }
  ...
  const completeArtifacts = artifacts.filter((artifact) => artifact !== void 0);
...
function xorshift324(value) {
  let x = value || 1831565813;
  x ^= x << 13;
  x ^= x >>> 17;
  x ^= x << 5;
  return x >>> 0;
...
function validate(message) {
  ...
  if (!Number.isSafeInteger(message.width) || !Number.isSafeInteger(message.height) || message.width <= 0 || message.height <= 0) throw new Error("Invalid output raster");
  if (message.sampleRate !== 48e3) throw new Error("Offline render input must be normalized to 48 kHz");
  if (!Number.isSafeInteger(message.fpsNum) || !Number.isSafeInteger(message.fpsDen) || message.fpsNum <= 0 || message.fpsDen <= 0) throw new Error("Invalid output frame rate");
  ...
  if (message.left.byteLength !== message.totalSamples * 4 || message.right.byteLength !== message.totalSamples * 4) {
  ...
  for (const cue of message.presetCues) {
    if (!Number.isSafeInteger(cue.frame) || cue.frame <= prior || !Number.isSafeInteger(cue.transitionFrames) || cue.transitionFrames < 0) {
...
function blendPackedRgb(from, to, mix, output) {
  const alpha = Math.max(0, Math.min(256, Math.round(mix * 256)));
  const inverse = 256 - alpha;
  for (let i = 0; i < output.length; i++) {
    const a = from[i];
    const b = to[i];
    const r = (a >>> 16) * inverse + (b >>> 16) * alpha >>> 8 & 255;
    const g = (a >>> 8 & 255) * inverse + (b >>> 8 & 255) * alpha >>> 8 & 255;
    const blue = (a & 255) * inverse + (b & 255) * alpha >>> 8 & 255;
    output[i] = r << 16 | g << 8 | blue;
  }
}
function fillFramePcm(left, right, sampleStart, sampleEnd, pcmLeft, pcmRight) {
  const available = Math.max(1, sampleEnd - sampleStart);
  for (let i = 0; i < AVS_SAMPLES; i++) {
    const source = Math.min(sampleEnd - 1, sampleStart + Math.trunc(i * available / AVS_SAMPLES));
...
async function refuseExistingFile(directory, name) {
  try {
