...
function buildAvsColorMapTable(points) {
  ...
  table2.fill(first.color & 16777215, 0, firstPosition);
  for (let i = 0; i + 1 < sorted.length; i++) {
    ...
    for (let position = leftPosition; position <= rightPosition; position++) {
      ...
      table2[position] = mix256(left.color, right.color, weight);
      fraction += increment;
    }
  }
  const last = sorted[sorted.length - 1];
  table2.fill(last.color & 16777215, clamp(last.position, 0, 255), 256);
  return table2;
}
function registerAvsColorMap(registry = new AvsEffectRegistry()) {
  ...
  registry.registerApe(AVS_COLOR_MAP_APE_ID, (context) => {
    ...
    const table2 = selectTable(config, state, context.beat);
    transform(context, config, table2);
  });
  return registry;
}
function selectTable(config, state, beat) {
  ...
  for (let i = 0; i < 256; i++) table2[i] = mix256(previous[i], target[i], state.progress);
  return table2;
}
function transform(context, config, table2) {
...
function blend(source, destination, mode, amount) {
  switch (mode) {
    case 0:
      return source;
    case 1:
      return channels2(source, destination, (s, d) => Math.min(255, s + d));
    case 2:
      return channels2(source, destination, Math.max);
    case 3:
      return channels2(source, destination, Math.min);
    case 4:
      return channels2(source, destination, (s, d) => s + d >>> 1);
    case 5:
      return channels2(source, destination, (s, d) => Math.max(0, d - s));
    case 6:
      return channels2(source, destination, (s, d) => Math.max(0, s - d));
    case 7:
      return channels2(source, destination, (s, d) => s * d >>> 8);
    case 8:
      return (source ^ destination) & 16777215;
    case 9:
      return channels2(source, destination, (s, d) => Math.min(255, (s * amount >>> 8) + (d * (256 - amount) >>> 8)));
    default:
      return destination;
  }
}
...
// src/avs/effects/color-map-gpu.ts
var AVS_GPU_COLOR_MAP_CAPABILITY = {
  id: "color-map-stable-lut-packed-u32",
  backend: "webgpu",
  lane: "exact",
  byteExact: true,
...
function assessExactGpuColorMap(config) {
...
var ExactAvsColorMapGpuPass = class {
  capability = AVS_GPU_COLOR_MAP_CAPABILITY;
  pipeline;
  table;
  ...
  constructor(device, config) {
    ...
    this.table = device.createBuffer({
      label: "AVS Color Map stable LUT",
      size: values.byteLength,
      usage: GPUBufferUsage.STORAGE,
      mappedAtCreation: true
    });
    new Uint32Array(this.table.getMappedRange()).set(values);
    this.table.unmap();
    const module = device.createShaderModule({ label: "AVS exact Color Map", code: buildExactAvsColorMapWgsl(config) });
    this.pipeline = device.createComputePipeline({
      label: `AVS exact Color Map key ${config.key} blend ${config.blendMode}`,
      layout: "auto",
...
function buildExactAvsColorMapWgsl(config) {
  const eligibility = assessExactGpuColorMap(config);
  if (!eligibility.eligible) throw new Error(eligibility.reason);
  return (
    /* wgsl */
...
function keyBody(key) {
  switch (key) {
    case 0:
      return "let key=red;";
    case 1:
      return "let key=green;";
    case 2:
      return "let key=blue;";
    case 3:
      return "let key=min(255u,(red+green+blue)>>1u);";
    case 4:
      return "let key=max(red,max(green,blue));";
    case 5:
      return "let key=(red+green+blue)/3u;";
    default:
      throw new RangeError(`Unsupported Color Map key ${key}`);
  }
}
function blendBody(mode, amount) {
  switch (mode) {
    ...
    case 4:
      return "return pack((sb+db)>>1u,(sg+dg)>>1u,(sr+dr)>>1u);";
    case 5:
    ...
    case 7:
      return "return pack((sb*db)>>8u,(sg*dg)>>8u,(sr*dr)>>8u);";
    case 8:
      return "return (source^destination)&0x00ffffffu;";
    case 9: {
      const inverse = 256 - amount;
      return `return pack(min(255u,((sb*${amount}u)>>8u)+((db*${inverse}u)>>8u)),min(255u,((sg*${amount}u)>>8u)+((dg*${inverse}u)>>8u)),min(255u,((sr*${amount}u)>>8u)+((dr*${inverse}u)>>8u)));`;
    }
    default:
      throw new RangeError(`Unsupported Color Map blend ${mode}`);
  }
}
...
// tools/avs-color-map-gpu-browser-check.ts
...
async function run() {
  ...
  let exactPixels = 0;
  for (const config2 of fixtures()) {
    const source2 = randomPixels(31 * 19);
    await differential(device, config2, 31, 19, source2);
    exactPixels += source2.length;
  }
  const width = 640, height = 360, source = randomPixels(width * height), config = fixture(4, 0), expected = cpuFrame(source, config, width, height), bytes = source.byteLength;
  const input = device.createBuffer({ size: bytes, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }), output = device.createBuffer({ size: bytes, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC }), pass = new ExactAvsColorMapGpuPass(device, config);
  const cpuSamples = [], gpuSamples = [], cpuBatchSamples = [], gpuBatchSamples = [], batchFrames = 30;
  for (let sample = 0; sample < 9; sample++) {
    ...
    started = performance.now();
    cpuBatch(source, config, width, height, batchFrames);
    cpuBatchSamples.push((performance.now() - started) / batchFrames);
    device.queue.writeBuffer(input, 0, source);
    encoder = device.createCommandEncoder();
    started = performance.now();
    for (let frame = 0; frame < batchFrames; frame++) pass.encode({ device, encoder, width, height, source: frame % 2 ? output : input, target: frame % 2 ? input : output });
    device.queue.submit([encoder.finish()]);
    await device.queue.onSubmittedWorkDone();
    gpuBatchSamples.push((performance.now() - started) / batchFrames);
  }
  finish({ pass: true, width, height, exactPixels, cpuMedianMs: median(cpuSamples), gpuMedianMs: median(gpuSamples), speedup: median(cpuSamples) / median(gpuSamples), cpuBatchMedianMs: median(cpuBatchSamples), gpuResidentBatchMedianMs: median(gpuBatchSamples), residentBatchSpeedup: median(cpuBatchSamples) / median(gpuBatchSamples), batchFrames, lutBytes: 1024, cpuSamples, gpuSamples, cpuBatchSamples, gpuBatchSamples, checksum: expected.reduce((sum, value) => Math.imul(sum ^ value, 16777619) >>> 0, 2166136261) });
}
async function differential(device, config, width, height, source) {
  const bytes = source.byteLength, input = device.createBuffer({ size: bytes, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }), output = device.createBuffer({ size: bytes, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC }), read = device.createBuffer({ size: bytes, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ }), pass = new ExactAvsColorMapGpuPass(device, config);
  device.queue.writeBuffer(input, 0, source);
  ...
  const actual = new Uint32Array(read.getMappedRange()), expected = cpuFrame(source, config, width, height);
  for (let i = 0; i < actual.length; i++) if (actual[i] !== expected[i]) throw new Error(`key ${config.key} blend ${config.blendMode} pixel ${i}: ${actual[i].toString(16)} != ${expected[i].toString(16)}`);
  read.unmap();
  input.destroy();
  output.destroy();
  read.destroy();
...
function fixtures() {
  return [...Array.from({ length: 6 }, (_, key) => fixture(key, 0)), ...Array.from({ length: 10 }, (_, blend2) => fixture(4, blend2))];
}
function fixture(key, blendMode) {
  const points = [{ position: 0, color: 1056816, id: 0 }, { position: 73, color: 16711808, id: 1 }, { position: 191, color: 57456, id: 2 }, { position: 255, color: 15790112, id: 3 }];
  return { key, blendMode, mapCycleMode: 0, adjustBlend: 173, dontSkipFastBeats: false, cycleSpeed: 8, maps: Array.from({ length: 8 }, (_, index) => ({ index, enabled: index === 0, id: index, filename: "", points })) };
}
function cpuFrame(source, config, width, height) {
  const payload = encode(config), component = { effectId: 16384, apeId: AVS_COLOR_MAP_APE_ID, payload, fileOffset: 0, path: "0", children: [], list: null, listCode: null }, preset = { version: 2, header: "Nullsoft AVS Preset 0.2", clearEveryFrame: false, components: [component], byteLength: payload.length }, audio = { waveform: [new Uint8Array(AVS_AUDIO_SAMPLES), new Uint8Array(AVS_AUDIO_SAMPLES)], spectrum: [new Uint8Array(AVS_AUDIO_SAMPLES), new Uint8Array(AVS_AUDIO_SAMPLES)], beat: false, beatLevel: 0 }, frame = new AvsFramebuffer(width, height, new Uint32Array(source));
  new AvsExecutor(preset, registerAvsColorMap()).render(frame, audio);
  return new Uint32Array(frame.pixels);
}
function cpuBatch(source, config, width, height, frames) {
  const payload = encode(config), component = { effectId: 16384, apeId: AVS_COLOR_MAP_APE_ID, payload, fileOffset: 0, path: "0", children: [], list: null, listCode: null }, preset = { version: 2, header: "Nullsoft AVS Preset 0.2", clearEveryFrame: false, components: [component], byteLength: payload.length }, audio = { waveform: [new Uint8Array(AVS_AUDIO_SAMPLES), new Uint8Array(AVS_AUDIO_SAMPLES)], spectrum: [new Uint8Array(AVS_AUDIO_SAMPLES), new Uint8Array(AVS_AUDIO_SAMPLES)], beat: false, beatLevel: 0 }, frame = new AvsFramebuffer(width, height, new Uint32Array(source)), executor = new AvsExecutor(preset, registerAvsColorMap());
  for (let index = 0; index < frames; index++) executor.render(frame, audio);
}
function encode(config) {
  const result = new Uint8Array(496 + config.maps.reduce((sum, map) => sum + map.points.length * 12, 0)), view = new DataView(result.buffer);
  view.setInt32(0, config.key, true);
  view.setInt32(4, config.blendMode, true);
  view.setInt32(8, config.mapCycleMode, true);
  result[12] = config.adjustBlend;
  result[14] = config.dontSkipFastBeats ? 1 : 0;
  result[15] = config.cycleSpeed;
  let tail = 496;
  config.maps.forEach((map, index) => {
    ...
    map.points.forEach((point) => {
      view.setUint32(tail, point.position, true);
      view.setUint32(tail + 4, point.color, true);
      view.setUint32(tail + 8, point.id, true);
      tail += 12;
    });
  });
  return result;
