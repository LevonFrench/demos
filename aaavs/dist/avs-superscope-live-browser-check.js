...
// tools/avs-superscope-live-browser-check.ts
var preset = "/avs%20presets/presets/unique/0021cb92ee07d779--SuperScope%20-%20Rotating%20Bow%20Thing.avs";
void run().catch((error) => finish({ error: error instanceof Error ? error.stack : String(error) }));
async function run() {
  const response = await fetch(preset);
  if (!response.ok) throw new Error(`preset fetch ${response.status}`);
  const bytes = new Uint8Array(await response.arrayBuffer());
  const enhanced = await render(bytes, "120");
  const exact = await render(bytes, "exact");
  if (enhanced.presenter !== "webgpu-enhanced" || !enhanced.gpuEnhancedSuperScope) {
    throw new Error(`enhanced lane did not activate: ${JSON.stringify(enhanced)}`);
  }
  if (exact.presenter === "webgpu-enhanced" || exact.gpuEnhancedSuperScope) {
    throw new Error(`exact lane crossed semantic boundary: ${JSON.stringify(exact)}`);
  }
  finish({ pass: true, preset, enhanced, exact });
}
async function render(bytes, gpuLane) {
  ...
  const renderer = new AvsWorkerRenderer({ canvas, context, gpuLane, onFrame: complete });
  await renderer.load(new Uint8Array(bytes), 320, 180);
  const waveform = new Float32Array(2048);
  for (let index = 0; index < waveform.length; index++) waveform[index] = Math.sin(index * 0.071);
  const accepted = renderer.render({ waveform }, 320, 180);
  if (!accepted) throw new Error("worker rejected first frame");
  await Promise.race([frame, new Promise((_, reject) => setTimeout(() => reject(new Error("frame timeout")), 3e4))]);
  const result = {
    presenter: renderer.presenter,
    gpuEnhancedSuperScope: renderer.gpuEnhancedSuperScope,
    gpuEffectComponents: renderer.gpuEffectComponents,
    gpuEffectPasses: renderer.gpuEffectPasses,
    gpuEffectPlan: renderer.gpuEffectPlan,
    renderMs: renderer.lastRenderMs,
    effectMs: renderer.lastEffectMs,
    uploadMs: renderer.lastUploadMs,
    encodeSubmitMs: renderer.lastEncodeSubmitMs
  };
  renderer.dispose();
  return result;
}
