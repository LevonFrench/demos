...
// tools/avs-texer-ordered-gpu-browser-benchmark.ts
void run().then((result) => finish(result), (error) => finish({ error: message(error) }));
async function run() {
  ...
  const bitmap = { width: 5, height: 5, pixels: Uint32Array.from({ length: 25 }, (_, i) => Math.imul(i + 1, 197895) & 16777215) };
  const source = Uint32Array.from({ length: width * height }, (_, i) => Math.imul(i + 1, 66309) & 16777215);
  ...
  for (const count of [128, 1024, 4096]) {
    ...
    const records = Array.from({ length: count }, (_, i) => buildExactAvsTexerRecord(bitmap, i * 97 % width, i * 193 % height, Math.imul(i + 1, 461581) & 16777215));
    const frame = { records, blendMode: 7, adjustableAlpha: 173, clearInput: false };
    const cpu = [];
    for (let i = 0; i < 5; i++) {
      const s = performance.now();
      renderExactAvsTexerRecordsCpu(source, width, height, bitmap, frame);
      cpu.push(performance.now() - s);
    }
    const gpu = [];
    const upload = [];
    for (let i = -3; i < 10; i++) {
    ...
    results.push({ records: count, membershipBytes: plan.tilePlan.membershipBytes, cpuRasterMedianMs: median(cpu), uploadCallMedianMs: median(upload), gpuEncodeSubmitCompleteMedianMs: median(gpu) });
    pass.destroy();
  }
  sourceBuffer.destroy();
  targetBuffer.destroy();
  device.destroy();
  return { width, height, measurement: "wall clock; GPU value includes encode, submit and queue completion", results };
}
function median(v) {
  return v.sort((a, b) => a - b)[Math.floor(v.length / 2)];
}
function message(e) {
  return e instanceof Error ? e.stack ?? e.message : typeof e === "object" && e !== null && "message" in e ? String(e.message) : String(e);
}
function finish(result) {
  document.documentElement.dataset.done = "true";
  const pre = document.querySelector("pre") ?? document.body.appendChild(document.createElement("pre"));
