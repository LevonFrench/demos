...
function buildExactAvsTexer2UnscaledRecord(bitmap, frameWidth, frameHeight, x, y, color, colorize, flipX, flipY) {
...
function renderExactAvsTexerRecordsCpu(source, width, height, bitmap, frame) {
  ...
  for (const record of frame.records) {
    ...
    for (let y = minY; y <= maxY; y++) for (let x = minX; x <= maxX; x++) {
      ...
      let sample = bitmap.pixels[sy * bitmap.width + sx] & 16777215;
...
function blendTexerPixel(source, destination, mode, amount) {
  ...
  const channels = (fn) => {
    const c = (shift) => fn(source >>> shift & 255, destination >>> shift & 255) & 255;
    return c(0) | c(8) << 8 | c(16) << 16;
  };
  switch (mode) {
    case 1:
      return channels((s, d) => Math.min(255, s + d));
    case 2:
      return channels(Math.max);
    case 3:
      return (source >>> 1 & 8355711) + (destination >>> 1 & 8355711);
    case 4:
      return channels((s, d) => Math.max(0, d - s));
    case 5:
      return channels((s, d) => Math.max(0, s - d));
    case 6:
      return channels((s, d) => s * d >>> 8);
    case 7: {
      const alpha = clamp(amount, 0, 255);
      return channels((s, d) => (s * alpha >>> 8) + (d * (256 - alpha) >>> 8));
    }
    case 8:
      return (source ^ destination) & 16777215;
    case 9:
      return channels(Math.min);
    default:
      return source;
  }
}
...
// tools/avs-texer-ordered-gpu-browser-check.ts
void run().then((message) => finish("pass", message), (error) => finish("fail", error instanceof Error ? error.stack ?? error.message : typeof error === "object" && error !== null && "message" in error ? String(error.message) : String(error)));
async function run() {
  ...
  const bitmap = { width: 5, height: 5, pixels: Uint32Array.from({ length: 25 }, (_, i) => i * 41 & 255 | (i * 73 & 255) << 8 | (i * 109 & 255) << 16) };
  const records = [
    buildExactAvsTexerRecord(bitmap, 3, 3, null),
    buildExactAvsTexerRecord(bitmap, 3, 3, 7385312),
    buildExactAvsTexerRecord(bitmap, 17, 15, 16777215)
  ];
  for (const args of [[0, 0, true, false], [-1, 1, false, true], [0.01, -0.02, true, true]]) {
    const record = buildExactAvsTexer2UnscaledRecord(bitmap, width, height, args[0], args[1], 13656208, true, args[2], args[3]);
    if (record) records.push(record);
  }
  const source = Uint32Array.from({ length: width * height }, (_, i) => Math.imul(i + 11, 66311) & 16777215);
  const plan = planExactAvsTexerGpu(width, height, bitmap, 128);
  if (!plan.eligible) throw new Error(plan.reason ?? "plan failed");
  device.pushErrorScope("validation");
  const pass = new ExactAvsOrderedTexerGpuPass(device, plan, bitmap);
  const creationError = await device.popErrorScope();
  if (creationError) throw creationError;
  const sourceBuffer = device.createBuffer({ size: source.byteLength, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST });
  const targetBuffer = device.createBuffer({ size: source.byteLength, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC });
  const readback = device.createBuffer({ size: source.byteLength, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });
  device.queue.writeBuffer(sourceBuffer, 0, source.buffer, source.byteOffset, source.byteLength);
  let assertions = 0;
  for (const blendMode of [0, 1, 3, 4, 6, 7, 8, 9]) {
    const frame = { records, blendMode, adjustableAlpha: 173, clearInput: blendMode === 3 };
    pass.update(frame);
    const encoder = device.createCommandEncoder();
    pass.encode({ device, encoder, width, height, source: sourceBuffer, target: targetBuffer });
    encoder.copyBufferToBuffer(targetBuffer, 0, readback, 0, source.byteLength);
    device.queue.submit([encoder.finish()]);
    await readback.mapAsync(GPUMapMode.READ);
    const actual = new Uint32Array(readback.getMappedRange().slice(0));
    readback.unmap();
    const expected = renderExactAvsTexerRecordsCpu(source, width, height, bitmap, frame);
    const mismatch = expected.findIndex((value, index) => value !== actual[index]);
    if (mismatch >= 0) throw new Error(`GPU mismatch for blend ${blendMode} at ${mismatch}: expected=${expected[mismatch].toString(16)} actual=${actual[mismatch].toString(16)}`);
    assertions++;
  }
  ...
  readback.destroy();
  device.destroy();
  return `PASS ${assertions} exact ordered Texer GPU differentials`;
}
